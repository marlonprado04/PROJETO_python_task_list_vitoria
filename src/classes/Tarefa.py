...
class Tarefa:
    ...
    # Adiciona um novo usuário no fim da lista se ele não existir
    def adicionar_usuario(self, id):
        # Converte ID em int
        id = int(id)

        # Verifica se o id do usuário não está na lista de ID
        if id not in self.id_usuarios:
            # Adiciona o ID do usuário na lista de ID
            self.id_usuarios.append(id)

    # Remove o usuário na lista de usuários da tarefa
    def remover_usuario(self, id):
        # Converte ID em int
        id = int(id)

        # Remove o ID do item
        self.id_usuarios.remove(id)
    ...
    def to_line(cls, dados):
        ...
        titulo_str = (
            f"Titulo: {dados['titulo']}" if "titulo" in dados else "Titulo: N/A"
        )
        descricao_str = (
        ...
        data_criacao_str = (
            f"Criada em: {dados['data_criacao']}"
            if "data_criacao" in dados
            else "Criada em: N/A"
        )
        status_str = (
            f"Status: {dados['status']}" if "status" in dados else "Status: N/A"
        )
        data_conclusao_str = (
            f"Concluída em: {dados['data_conclusao']}"
            if "data_conclusao" in dados
            else "Concluída em: N/A"
        )
        id_usuarios_str = (
            f"ID Usuários: {dados['id_usuarios']}"
            if "id_usuarios" in dados
            else "ID Usuários: N/A"
        )
        id_projetos_str = (
